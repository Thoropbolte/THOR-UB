...
from userbot import *
from userbot import bot as thorbot
...
@thorbot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@thorbot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {mafia}\n\n⚡ **[ʟᴇɢᴇɴᴅᴀʀʏ ᴀғ ᴍᴀғɪᴀʙᴏᴛ](t.me/ThorBot_Support)** ⚡"
...
@thorbot.on(admin_cmd(pattern="install$", outgoing=True))
@thorbot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {mafia}\n\n{string}\n\n        ⚡ **[ʟᴇɢᴇɴᴅᴀʀʏ ᴀғ ᴍᴀғɪᴀʙᴏᴛ](t.me/ThorBot_Support)** ⚡", link_preview=False)
...
@thorbot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@thorbot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@thorbot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@thorbot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@thorbot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@thorbot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in thorbot."
).add()
