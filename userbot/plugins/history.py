...
from thorbot import bot, CmdHelp
from thorbot.utils import admin_cmd, edit_or_reply as eor, sudo_cmd

@thorbot.on(admin_cmd(pattern="history ?(.*)"))
@thorbot.on(sudo_cmd(pattern="history ?(.*)", allow_sudo=True))
...
@thorbot.on(admin_cmd(pattern="unh ?(.*)"))
@thorbot.on(sudo_cmd(pattern="unh ?(.*)", allow_sudo=True))
async def _(mafiaevent):
    if mafiaevent.fwd_from:
        return 
    if not mafiaevent.reply_to_msg_id:
