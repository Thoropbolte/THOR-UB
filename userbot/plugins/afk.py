...
# Now will be used in ThorBot too....
import asyncio
import datetime
...
from userbot import CMD_HELP, thorversion
from thorbot.utils import admin_cmd, edit_or_reply
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        thorbot = await bot.send_message(
            event.chat_id,
            "🔥__Back alive!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`", file=thorpic
        ...
        try:
            await bot.send_message(  # pylint:disable=E0602
                Config.THORBOT_LOGGER,  # pylint:disable=E0602
            ...
            await bot.send_message(  # pylint:disable=E0602
                event.chat_id,
                "Please set `THORBOT_LOGGER` "
                + "for the proper functioning of afk functionality "
                + "Ask in @ThorBot_Chit_Chat to get help setting this value\n\n `{}`".format(str(e)),
                reply_to=event.message.id,
                silent=True,
            )
        await asyncio.sleep(5)
        await thorbot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        message_to_reply = (
            f"Hey!! My Legend master [{DEFAULTUSER}](tg://user?id={thor}) is currently offline... Since when?\n**For** `{total_afk_time}`\n"
            + f"\n\n👇__The Reason Is__👇 :-\n`{reason}`"
  if reason
            else f"**Heyy!**\n__I am currently unavailable.__\n__Since when, you ask? From__ `{total_afk_time}`\nI'll be back when I feel to come🚶"
        )
        msg = await event.reply(message_to_reply, file=thorpic)
...
async def _(event):
    ...
    thorpic = await event.client.download_media(h1m4n5hu0p)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {thorpic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"__**I'm going afk🚶**__ \n⚜️ Because `{reason}`", file=thorpic
            )
        else:
            await bot.send_message(event.chat_id, f"**I am Going afk!**🚶", file=thorpic)
        await asyncio.sleep(0.001)
        await event.delete()
        try:
            await bot.send_message(  # pylint:disable=E0602
                Config.THORBOT_LOGGER,  # pylint:disable=E0602
                f"#AFKTRUE \nSet AFK mode to True, and Reason is {reason}",file=thorpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
