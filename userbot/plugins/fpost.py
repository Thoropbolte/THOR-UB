...
async def all_groups_id(thor):
    thorgroups = []
    async for dialog in thor.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            thorgroups.append(entity.id)
    return thorgroups
...
async def _(event):
    ...
    if Config.THORBOT_LOGGER is None:
        await edit_or_reply(
            event,
            "Please set the required environment variable `THORBOT_LOGGER` for this plugin to work",
        )
        return
    try:
        e = await event.client.get_entity(Config.THORBOT_LOGGER)
