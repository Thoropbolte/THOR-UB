...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        thorbot = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        thorbot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await thorbot.edit(f"`🔥Global Ban Iz Cumin💦.... Wait and watch nigga🚶`")
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await thorbot.edit(f"Gbaning This retard🚶\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await thorbot.edit(f"`Either reply to a user or gib me user id/name`")
    try:        
        if gmute(user.id) is False:
            return await thorbot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await thorbot.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        thorbot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        thorbot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await thorbot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await thorbot.edit(f"Ok! Now Ungbaning this kiddo.\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await thorbot.edit("**Reply to a user**")     
    try:
        if ungmute(user.id) is False:
            return await thorbot.edit("**Error! I think User already ungbanned.**")
    except:
        pass
    return await thorbot.edit(
