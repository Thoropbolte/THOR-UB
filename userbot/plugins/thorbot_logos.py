...
#modifiy by @D3krish for Thorbot
#If you kang this plugin please don't forget to give credit to Thorbot and Hellbot. 
import os
import random
import time
...
async def _(thorevent):
    event = await edit_or_reply(thorevent, "`Processing.....`")
    fnt = await get_font_file(thorevent.client, "@D3VIL_FONTSS")
    if thorevent.reply_to_msg_id:
        rply = await thorevent.get_reply_message()
    ...
    text = thorevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "ThorBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        thorevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『[{DEFAULTUSER}](tg://user?id={thor})』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
