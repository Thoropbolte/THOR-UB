                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from thorbot.utils import register, errors_handler
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await thor.edit("U want to promote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await thor.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await thor.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await thor.edit(f"**Reply to a user you dumbo !!**")
    return await thor.edit(
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await thor.edit("U want to demote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await thor.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await thor.edit(f"**Reply to a user you dumbo !!**")
