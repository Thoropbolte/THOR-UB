...
async def _(event):
    ...
    if tgbotusername is not None or thor_input == "text":
        results = await event.client.inline_query(tgbotusername, "@ThorBot_Support")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of ThorBot\n\n"
        )
        thorcount = 0
        plugincount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=thorcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of the ThorBot are [here]({url})"
