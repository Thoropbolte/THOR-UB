...
from thorbot import bot
from thorbot.utils import admin_cmd, sudo_cmd, edit_or_reply
from thorbot.cmdhelp import CmdHelp
from thorbot.helpers.functions import deEmojify
...
async def nope(h1m4n5hu0p):
    thor = h1m4n5hu0p.pattern_match.group(1)
    if not thor:
    ...
    troll = await bot.inline_query("animedb_bot", f"{(deEmojify(thor))}")
