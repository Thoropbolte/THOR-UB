...
async def bigspam(thor):
    if not thor.text[0].isalpha() and thor.text[0] not in ("/", "#", "@", "!"):
        thor_msg = thor.text
        thorbot_count = int(thor_msg[9:13])
        thor_spam = str(thor.text[13:])
        for i in range(1, thorbot_count):
            await thor.respond(thor_spam)
        await thor.delete()
        if LOGGER:
            await thor.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what thorbot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
