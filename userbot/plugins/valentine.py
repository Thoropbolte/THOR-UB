...
h1m4n5hu0p = thorbot.uid

thor = f"[{DEFAULTUSER}](tg://user?id={h1m4n5hu0p})"

@thorbot.on(admin_cmd(pattern="hvd$"))
@thorbot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@thorbot.on(admin_cmd(pattern="vday$"))
@thorbot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
